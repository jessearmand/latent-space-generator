...
export const ConfigProvider = ({ children }: { children: ReactNode }) => {
    ...
    const normalizedTtsVolume = Number.isFinite(storedTtsVolume) ? Math.min(10, Math.max(0.01, storedTtsVolume)) : 1.0;
    ...
    const [imageSize, setImageSize] = useState<{ width: number; height: number } | string>(
        JSON.parse(localStorage.getItem('IMAGE_SIZE') || '"landscape_4_3"'),
    );
    const [raw, setRaw] = useState<boolean>(localStorage.getItem('RAW') === 'true');
    const [enableSafetyChecker, setEnableSafetyChecker] = useState<boolean>(
        localStorage.getItem('ENABLE_SAFETY_CHECKER') === 'true',
    );
    const [seed, setSeed] = useState<number | null>(
        localStorage.getItem('SEED') !== 'null' ? parseInt(localStorage.getItem('SEED') || '0', 10) : null,
    );
    const [guidanceScale, setGuidanceScale] = useState<number>(
        parseFloat(localStorage.getItem('GUIDANCE_SCALE') || '3.5'),
    ); // Default from API
    const [imagePromptStrength, setImagePromptStrength] = useState<number>(
        parseFloat(localStorage.getItem('IMAGE_PROMPT_STRENGTH') || '0.1'),
    ); // Default for image-editing
    const [gptImageSize, setGptImageSize] = useState<string>(localStorage.getItem('GPT_IMAGE_SIZE') || 'auto');
    const [gptNumImages, setGptNumImages] = useState<number>(
        parseInt(localStorage.getItem('GPT_NUM_IMAGES') || '1', 10) || 1,
    );
    ...
    // Gemini image model settings
    const [geminiNumImages, setGeminiNumImages] = useState<number>(
        parseInt(localStorage.getItem('GEMINI_NUM_IMAGES') || '1', 10) || 1,
    );
    const [geminiOutputFormat, setGeminiOutputFormat] = useState<string>(
        localStorage.getItem('GEMINI_OUTPUT_FORMAT') || 'png',
    );
    const [geminiResolution, setGeminiResolution] = useState<string>(localStorage.getItem('GEMINI_RESOLUTION') || '1K');
    const [geminiEnableWebSearch, setGeminiEnableWebSearch] = useState<boolean>(
        localStorage.getItem('GEMINI_ENABLE_WEB_SEARCH') === 'true',
    );
    // Grok Imagine model settings
    const [grokNumImages, setGrokNumImages] = useState<number>(
        parseInt(localStorage.getItem('GROK_NUM_IMAGES') || '1', 10) || 1,
    );
    const [grokOutputFormat, setGrokOutputFormat] = useState<string>(
        localStorage.getItem('GROK_OUTPUT_FORMAT') || 'jpeg',
    );
    // Qwen model settings
    ...
    const [videoAspectRatio, setVideoAspectRatio] = useState<string>(
        localStorage.getItem('VIDEO_ASPECT_RATIO') || '16:9',
    );
    const [videoResolution, setVideoResolution] = useState<string>(localStorage.getItem('VIDEO_RESOLUTION') || '720p');
    const [videoGuidanceScale, setVideoGuidanceScale] = useState<number>(
        parseFloat(localStorage.getItem('VIDEO_GUIDANCE_SCALE') || '3'),
    );
    const [videoSeed, setVideoSeed] = useState<number | null>(
        localStorage.getItem('VIDEO_SEED') !== 'null' ? parseInt(localStorage.getItem('VIDEO_SEED') || '0', 10) : null,
    );
    ...
    const [videoCfgScale, setVideoCfgScale] = useState<number>(
    ...
    const [videoOutputSize, setVideoOutputSize] = useState<string>(
        localStorage.getItem('VIDEO_OUTPUT_SIZE') || 'landscape_4_3',
    );
    const [videoUseMultiscale, setVideoUseMultiscale] = useState<boolean>(
        localStorage.getItem('VIDEO_USE_MULTISCALE') !== 'false',
    ); // Default true
    const [videoNumInferenceSteps, setVideoNumInferenceSteps] = useState<number>(
        parseInt(localStorage.getItem('VIDEO_NUM_INFERENCE_STEPS') || '40', 10),
    );
    const [videoAcceleration, setVideoAcceleration] = useState<string>(
        localStorage.getItem('VIDEO_ACCELERATION') || 'regular',
    );
    const [videoCameraLora, setVideoCameraLora] = useState<string>(localStorage.getItem('VIDEO_CAMERA_LORA') || 'none');
    const [videoCameraLoraScale, setVideoCameraLoraScale] = useState<number>(
        parseFloat(localStorage.getItem('VIDEO_CAMERA_LORA_SCALE') || '1'),
    );
    const [videoEnablePromptExpansion, setVideoEnablePromptExpansion] = useState<boolean>(
        localStorage.getItem('VIDEO_ENABLE_PROMPT_EXPANSION') === 'true',
    );
    // Video-to-video settings
    ...
    const [audioSeed, setAudioSeed] = useState<number | null>(
        localStorage.getItem('AUDIO_SEED') !== 'null' ? parseInt(localStorage.getItem('AUDIO_SEED') || '0', 10) : null,
    );
    // TTS settings (MiniMax Speech models)
    const [ttsVoiceId, setTtsVoiceId] = useState<string>(
        storedTtsVoiceId && isValidMinimaxVoiceId(storedTtsVoiceId) ? storedTtsVoiceId : MINIMAX_DEFAULT_VOICE_ID,
    );
    ...
    // Chatterbox TTS settings
    const [chatterboxExaggeration, setChatterboxExaggeration] = useState<number>(
        parseFloat(localStorage.getItem('CHATTERBOX_EXAGGERATION') || '0.25'),
    );
    const [chatterboxTemperature, setChatterboxTemperature] = useState<number>(
        parseFloat(localStorage.getItem('CHATTERBOX_TEMPERATURE') || '0.7'),
    );
    const [chatterboxCfg, setChatterboxCfg] = useState<number>(
        parseFloat(localStorage.getItem('CHATTERBOX_CFG') || '0.5'),
    );
    // Music/SFX generation settings
    const [audioDuration, setAudioDuration] = useState<number>(
        parseFloat(localStorage.getItem('AUDIO_DURATION') || '10'),
    );
    // Beatoven-specific settings
    const [beatovenRefinement, setBeatovenRefinement] = useState<number>(
        parseInt(localStorage.getItem('BEATOVEN_REFINEMENT') || '100', 10),
    );
    const [beatovenCreativity, setBeatovenCreativity] = useState<number>(
        parseFloat(localStorage.getItem('BEATOVEN_CREATIVITY') || '16'),
    );
    const [beatovenNegativePrompt, setBeatovenNegativePrompt] = useState<string>(
        localStorage.getItem('BEATOVEN_NEGATIVE_PROMPT') || '',
    );
    // MMAudio V2 settings
    const [mmAudioCfgStrength, setMmAudioCfgStrength] = useState<number>(
        parseFloat(localStorage.getItem('MMAUDIO_CFG_STRENGTH') || '4.5'),
    );
    const [mmAudioNumSteps, setMmAudioNumSteps] = useState<number>(
        parseInt(localStorage.getItem('MMAUDIO_NUM_STEPS') || '25', 10),
    );
    // Bria BG Removal settings
    const [briaBgColor, setBriaBgColor] = useState<string>(localStorage.getItem('BRIA_BG_COLOR') || 'transparent');
    const [briaOutputCodec, setBriaOutputCodec] = useState<string>(
        localStorage.getItem('BRIA_OUTPUT_CODEC') || 'mp4_h265',
    );
    // Video-to-video preprocessor
    const [videoPreprocessor, setVideoPreprocessor] = useState<string>(
        localStorage.getItem('VIDEO_PREPROCESSOR') || 'none',
    );
    // Audio understanding settings
    const [audioDetailedAnalysis, setAudioDetailedAnalysis] = useState<boolean>(
        localStorage.getItem('AUDIO_DETAILED_ANALYSIS') === 'true',
    );
    // MiniMax Speech 2.8 Turbo settings
    const [minimaxLanguageBoost, setMinimaxLanguageBoost] = useState<string>(
        localStorage.getItem('MINIMAX_LANGUAGE_BOOST') || 'auto',
    );
    // Qwen3-TTS settings
    ...
    const [qwenTtsStylePrompt, setQwenTtsStylePrompt] = useState<string>(
        localStorage.getItem('QWEN_TTS_STYLE_PROMPT') || '',
    );
    const [qwenTtsTemperature, setQwenTtsTemperature] = useState<number>(
        parseFloat(localStorage.getItem('QWEN_TTS_TEMPERATURE') || '0.9'),
    );
    // ElevenLabs TTS settings
    const [elevenLabsVoice, setElevenLabsVoice] = useState<string>(
        localStorage.getItem('ELEVENLABS_VOICE') || 'Rachel',
    );
    const [elevenLabsStability, setElevenLabsStability] = useState<number>(
        parseFloat(localStorage.getItem('ELEVENLABS_STABILITY') || '0.5'),
    );
    const [elevenLabsSimilarityBoost, setElevenLabsSimilarityBoost] = useState<number>(
        parseFloat(localStorage.getItem('ELEVENLABS_SIMILARITY_BOOST') || '0.75'),
    );
    const [elevenLabsStyle, setElevenLabsStyle] = useState<number>(
        parseFloat(localStorage.getItem('ELEVENLABS_STYLE') || '0'),
    );
    const [elevenLabsSpeed, setElevenLabsSpeed] = useState<number>(
        parseFloat(localStorage.getItem('ELEVENLABS_SPEED') || '1'),
    );
    // ElevenLabs SFX v2 settings
    const [elevenLabsPromptInfluence, setElevenLabsPromptInfluence] = useState<number>(
        parseFloat(localStorage.getItem('ELEVENLABS_PROMPT_INFLUENCE') || '0.3'),
    );
    // ElevenLabs Music settings
    const [elevenLabsForceInstrumental, setElevenLabsForceInstrumental] = useState<boolean>(
        localStorage.getItem('ELEVENLABS_FORCE_INSTRUMENTAL') === 'true',
    );
    // PersonaPlex settings
    const [personaPlexVoice, setPersonaPlexVoice] = useState<string>(
        localStorage.getItem('PERSONAPLEX_VOICE') || 'NATF2',
    );
    ...
    useEffect(() => {
        ...
        beatovenRefinement,
        beatovenCreativity,
        beatovenNegativePrompt,
        mmAudioCfgStrength,
        mmAudioNumSteps,
        briaBgColor,
        briaOutputCodec,
        videoPreprocessor,
        audioDetailedAnalysis,
    ...
    return (
        <ConfigContext.Provider
            value={{
                ...
                setGeminiNumImages,
                geminiOutputFormat,
                setGeminiOutputFormat,
                geminiResolution,
                setGeminiResolution,
                geminiEnableWebSearch,
                setGeminiEnableWebSearch,
                grokNumImages,
                setGrokNumImages,
                grokOutputFormat,
                setGrokOutputFormat,
                ...
                setVideoCameraLora,
                videoCameraLoraScale,
                setVideoCameraLoraScale,
                videoEnablePromptExpansion,
                setVideoEnablePromptExpansion,
                // Video-to-video settings
                videoStrength,
                setVideoStrength,
                // Audio generation
                ...
                // Beatoven settings
                beatovenRefinement,
                setBeatovenRefinement,
                beatovenCreativity,
                setBeatovenCreativity,
                beatovenNegativePrompt,
                setBeatovenNegativePrompt,
                // MMAudio V2 settings
                mmAudioCfgStrength,
                setMmAudioCfgStrength,
                mmAudioNumSteps,
                setMmAudioNumSteps,
                // Bria BG Removal settings
                briaBgColor,
                ...
                // Video preprocessor
                videoPreprocessor,
                setVideoPreprocessor,
                // Audio understanding
                audioDetailedAnalysis,
                setAudioDetailedAnalysis,
                ...
                setQwenTtsLanguage,
                qwenTtsStylePrompt,
                setQwenTtsStylePrompt,
                qwenTtsTemperature,
                setQwenTtsTemperature,
                // ElevenLabs TTS
                elevenLabsVoice,
                setElevenLabsVoice,
                elevenLabsStability,
                ...
                // ElevenLabs SFX v2
                elevenLabsPromptInfluence,
                setElevenLabsPromptInfluence,
                // ElevenLabs Music
                elevenLabsForceInstrumental,
                setElevenLabsForceInstrumental,
                // PersonaPlex
                personaPlexVoice,
                setPersonaPlexVoice,
            }}
