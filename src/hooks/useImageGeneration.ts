...
import { buildGrokImageInput, buildGenericImageInput, buildImageInputParams } from '../services/imageInputBuilders';
    ...
    const generateImage = useCallback(
        async (prompt: string, model: ModelConfig) => {
            ...
            if (isGeminiModel) {
                ...
                try {
                    const result = await routeGeminiImage(
                        modelId,
                        modelName,
                        {
                            prompt,
            ...
            const imageInputParams =
                supportsImageInput && uploadedImageUrls.length > 0
